...
Abstract:
    ...
    Implementation of Virtaul Audio Device data saving class.
...
//=============================================================================
NTSTATUS CSaveData::FileOpen(_In_ BOOL fOverWrite)
{
    PAGED_CODE();
    ...
    if (!m_FileHandle)
    {
        ntStatus = ZwCreateFile(&m_FileHandle, GENERIC_WRITE | SYNCHRONIZE, &m_objectAttributes, &ioStatusBlock, NULL, FILE_ATTRIBUTE_NORMAL,
            0, fOverWrite ? FILE_OVERWRITE_IF : FILE_OPEN_IF, FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
...
//=============================================================================
NTSTATUS CSaveData::FileWrite(_In_reads_bytes_(ulDataSize) PBYTE pData, _In_ ULONG ulDataSize)
{
    PAGED_CODE();
...
NTSTATUS CSaveData::SetDeviceObject(_In_ PDEVICE_OBJECT DeviceObject)
{
    PAGED_CODE();
...
NTSTATUS CSaveData::Initialize()
{
    ...
    // Create the folder.
    ntStatus = ZwCreateFile(&fileHandle, 0, &objectAttributes, &ioStatusBlock, NULL, FILE_ATTRIBUTE_NORMAL, 0,
    ...
    if (NT_SUCCESS(ntStatus))
    {
        ...
        m_FileName.Buffer = (PWSTR)ExAllocatePool2(POOL_FLAG_PAGED, m_FileName.MaximumLength, SAVEDATA_POOLTAG3);
        if (!m_FileName.Buffer)
        {
    ...
    if (NT_SUCCESS(ntStatus))
    {
        ...
        m_pDataBuffer = (PBYTE)ExAllocatePool2(POOL_FLAG_NON_PAGED, m_ulBufferSize, SAVEDATA_POOLTAG4);
        if (!m_pDataBuffer)
        {
    ...
    if (NT_SUCCESS(ntStatus))
    {
        m_fFrameUsed = (PBOOL)ExAllocatePool2(POOL_FLAG_NON_PAGED, m_ulFrameCount * sizeof(BOOL) + sizeof(LARGE_INTEGER), SAVEDATA_POOLTAG2);
        if (!m_fFrameUsed)
        {
    ...
    if (NT_SUCCESS(ntStatus))
    {
        ...
        ntStatus = KeWaitForSingleObject(&m_FileSync, Executive, KernelMode, FALSE, NULL);
        if (STATUS_SUCCESS == ntStatus)
        {
...
//=============================================================================
NTSTATUS CSaveData::InitializeWorkItems(_In_ PDEVICE_OBJECT DeviceObject)
{
    PAGED_CODE();
    ...
    m_pWorkItems = (PSAVEWORKER_PARAM)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(SAVEWORKER_PARAM) * MAX_WORKER_ITEM_COUNT, SAVEDATA_POOLTAG);
    if (m_pWorkItems)
    {
        for (int i = 0; i < MAX_WORKER_ITEM_COUNT; i++)
        {
...
VOID SaveFrameWorkerCallback(_In_ PDEVICE_OBJECT pDeviceObject, _In_opt_ PVOID Context)
{
    ...
    if (pParam->WorkItem)
    {
        ...
        if (STATUS_SUCCESS == KeWaitForSingleObject(&pSaveData->m_FileSync, Executive, KernelMode, FALSE, NULL))
        {
...
//=============================================================================
NTSTATUS CSaveData::SetDataFormat(_In_ PKSDATAFORMAT pDataFormat)
{
    PAGED_CODE();
    NTSTATUS ntStatus = STATUS_SUCCESS;
    ...
    if (pwfx)
    {
        ...
        m_waveFormat = (PWAVEFORMATEX)ExAllocatePool2(POOL_FLAG_NON_PAGED, (pwfx->wFormatTag == WAVE_FORMAT_PCM) ? sizeof(PCMWAVEFORMAT) :
            sizeof(WAVEFORMATEX) + pwfx->cbSize, SAVEDATA_POOLTAG1);
...
//=============================================================================
NTSTATUS CSaveData::SetMaxWriteSize(_In_ ULONG ulMaxWriteSize)
{
    PAGED_CODE();
    ...
    //
    buffer = (PBYTE)ExAllocatePool2(POOL_FLAG_NON_PAGED, bufferSize, SAVEDATA_POOLTAG4);
    if (!buffer)
    {
        DPF(D_TERSE, ("[Could not allocate memory for Saving Data]"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
...
//=============================================================================
void CSaveData::ReadData(_Inout_updates_bytes_all_(ulByteCount) PBYTE pBuffer, _In_ ULONG ulByteCount)
{
...
//=============================================================================
#pragma code_seg()
void CSaveData::SaveFrame(_In_ ULONG ulFrameNo, _In_ ULONG ulDataSize)
{
...
void CSaveData::WaitAllWorkItems(void)
{
    ...
    {
        ULONG size = m_ulBufferOffset - m_ulFrameIndex * m_ulFrameSize;
...
#pragma code_seg()
//=============================================================================
void CSaveData::WriteData(_In_reads_bytes_(ulByteCount) PBYTE pBuffer, _In_ ULONG ulByteCount)
{
    ASSERT(pBuffer);
    ...
    if (!m_fFrameUsed[m_ulFrameIndex])
    {
        ...
        if (ulWriteBytes != ulByteCount)
        {
            ...
            {
                KeReleaseSpinLock(&m_FrameInUseSpinLock, oldIrql);
                RtlCopyMemory(m_pDataBuffer + m_ulBufferOffset, pBuffer + ulWriteBytes, ulByteCount - ulWriteBytes);
