    ...
    public CUnknown    
{
    private:
        ...
        PCVirtualAudioDeviceHW   m_pHW;                  // Virtual Audio Device HW object
    ...
    public:
        ...
        ( 
            ...
            _In_reads_opt_(cPropertyCount) const VIRTUALAUDIODEVICE_DEVPROPERTY   * pProperties,
...
NTSTATUS VirtualAudioDeviceIoSetDeviceInterfacePropertyDataMultiple
(
    _In_ PUNICODE_STRING                                        SymbolicLinkName,
    _In_ ULONG                                                  cPropertyCount,
    _In_reads_opt_(cPropertyCount) const VIRTUALAUDIODEVICE_DEVPROPERTY        *pProperties
...
--*/
{
    ...
    // Initialize HW.
    // 
    m_pHW = new (POOL_FLAG_NON_PAGED, VIRTUALAUDIODEVICE_POOLTAG)  CVirtualAudioDeviceHW;
    if (!m_pHW)
    {
        DPF(D_TERSE, ("Insufficient memory for Virtual Audio Device HW"));
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
...
--*/
{
    ...
    //
    ntStatus = VirtualAudioDeviceIoSetDeviceInterfacePropertyDataMultiple(AudioSymbolicLinkName, cPropertyCount, pProperties);

    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: VirtualAudioDeviceIoSetDeviceInterfacePropertyDataMultiple(...): failed, 0x%x", ntStatus)),
        Done);
...
)
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology || NULL == unknownMiniTopo)
    {
        ...
        // Install Virtual Audio Device topology miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with VIRTUALAUDIODEVICE.<TopoName>.szPname in the inf's [Strings] section
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave || NULL == unknownMiniWave)
    {
        ...
        // Install Virtual Audio Device wave miniport for the render endpoint.
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with VIRTUALAUDIODEVICE.<WaveName>.szPname in the inf's [Strings] section
