...
)
{
    ...
    if (!m_bCapture && (!g_DoNotCreateDataFiles))
    {
        ...
        // Virtual Audio Device uses following buffer to hold data before writing to a file.
    ...
    switch (State_)
    {
        ...
        case KSSTATE_RUN:
            ...
            if (m_ulNotificationIntervalMs > 0)
            {
                ...
                // notification events only after notification interval. This timer is used by Virtual Audio Device to 
...
--*/
{
    ...
    //
    // Virtual Audio Device writes each stream seperately to disk. If the rights for this
